...
# Set resolution for vertical short video (9:16 aspect ratio, 840x720 resolution)
screen_width, screen_height = 640, 420
...
while cap.isOpened():
    ...
    # Rotate the frame 90° clockwise to correct the anti-clockwise rotation for inference
    ...
    if results.pose_landmarks:
        ...
        # Extract keypoints in the current video screen resolution
        keypoints = {
            landmark.name: (int(lm[landmark].x * w), int(lm[landmark].y * h))
            for landmark in mp_pose.PoseLandmark
        }
        
        # Save keypoints with frame index for JSON export
    ...
    # Continuously update the JSON file every 100 frames
    if frame_idx % 100 == 0:
...
# Final export of collected keypoints data to JSON file
with open("output_coordinates.json", "w") as outfile:
    json.dump(export_data, outfile, indent=4)
